...
/** useScrollSpy — P-CO-2.2 (2026-05-16). Self-contained window-scroll
 * copy (DashboardShell scrolls the document). Same as the product
 * hubs / 360 — kept local so النموّ tab stays decoupled. */
  ...
  useEffect(() => {
    ...
    function compute() {
      ...
      for (const id of sectionIds) {
        ...
        if (!el) continue;
        if (el.getBoundingClientRect().top + scrollTop <= line) current = id;
        else break;
      }
      setActiveId(current);
    }
